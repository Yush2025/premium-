...
def ethan():
       ...
       {BG}  {C}              ░╚════╝░╚══════╝░░░╚═╝░░░           {BG}         {RW}
       {BG}                                                             {RW}
       {BG}   {BG1}Uses       :{RW} {BG} {Q}BOOSTING TOOLS {BG}                             {RW}
       {BG}   {BG1}Version    : {RW}{BG} {Q}10.5  {BG}                                      {RW}
       {BG}   {BG1}Tools      :{RW} {BG}{Q} PREMIUM{BG}                                     {RW}
       {BG}   {BG1}Boost only :{RW} {BG}{lightblue} Facebook {BG}                                   {RW}
       {BG}   {BG1}Status     :{RW} {BG}{Q} online{Q}                                      {RW}
       {BG}   {BG1}Country    :{RW} {BG}{Q} Philippines {BG}                                {RW}
       {BG}                                                             {RW}
       {BG}             {BG1} DEVELOPER  {gh}ALFIE  TEOREMA    {RW}{BG}              {RW}
       {BG}                                                             {RW}""")
...
def perform_reaction_fast():
    ...
    clear_screen()
    ethan()
    print(f"     {red}[01] {blue}FRA ACCOUNT")
    print(f"     {red}[02] {blue}FRA PAGES")
    print(f"     {yellow}[03] {blue}RPW ACCOUNT")
    print(f"     {yellow}[04] {blue}RPW PAGES")
    print(f" {Q7}─────────────────────────────────────────────────────────────────\033[0m")
    try:
        file_choice = int(input(f"{green} CHOOSE HERE : ").strip())
        print(f" {Q7}─────────────────────────────────────────────────────────────────\033[0m")
        if file_choice not in file_options:
            print("Invalid choice.")
            return
    ...
    try:
        ...
        return
    tokens = tokens[start_line - 1:]
    print(f" {white}FORMAT : {red}https://www.facebook.com/100078043222260/posts/11017778267538/?mibextid=rS40aB7S9Ucbxw6v")
    print(f" {Q7}─────────────────────────────────────────────────────────────────\033[0m")
    z = input(f" {white}ENTER THE POST LINK OR ID SAME ON FORMAT: ")
...
def main():
    ...
    print(f"""       {BG}  {Z1} ༺═────────────────{BG1}ACCOUNT OVERVIEWS{RW}{BG}{Z1}───────────────────═༻  {BG}{RW}
       ...
       {BG}                                                             {RW} 
       {BG}                                                             {RW} """)
    print(f"       {BG} {hotpink}╭─────────────╮{RW}{BG}         {hotpink}  ╭─────────────╮{RW}{BG}    {hotpink}╭─────────────╮{RW}")
    print(f"       {BG} {hotpink}│ {Z1}SERVICES  {hotpink}  │{RW}{BG}        {hotpink}   │{Z1}  SELECTION  {hotpink}│{RW}{BG}    {hotpink}│ {Z1}DISCRIPTION {hotpink}│{RW}")
    print(f"       {BG} {hotpink}╰─────────────╯{RW}{BG}          {hotpink} ╰─────────────╯{RW}{BG}   {hotpink} ╰─────────────╯{RW}")
    print(f"       {BG}{B}  Account Extraction         {F}  [ {Z1}01 {F}]    {XX}    Extractor       {RW}")
    print(f"       {BG}{B}  Reaction for Post V1        {F} [ {Z1}02 {F}]     {XX}   Page Or Account {RW}")
    print(f"       {BG}{B}  Reaction for Reels        {F}   [ {Z1}03{F} ]       {XX} Page Or Account {RW}")
    print(f"       {BG}{B}  Reaction Group Page Post     {F}[{Z1} 04 {F}]     {XX}   Page Or Account {RW}")
    print(f"       {BG}{B}  Reaction Video Or Photo    {F}  [ {Z1}05{F} ]    {XX}    Page Or Account {RW}")
    print(f"       {BG}{B2}  Auto Followers           {F}    [ {Z1}06{F} ]      {XX}  Page Or Account {RW}")
    print(f"       {BG}{B2}  Reaction DP or Post         {F} [{Z1} 07 {F}]     {XX}   Page Or Account {RW}")
    print(f"       {BG}{B2}  Custom Comments          {F}    [ {Z1}08 {F}]      {XX}  Page Or Account {RW}")
    print(f"       {BG}{B2}  Page Likes and Followers    {F} [ {Z1}09 {F}]   {XX}Unstable but working { RW}")
    print(f"       {BG}{B2}  Reaction Dp Or Cover        {F} [ {Z1}10 {F}]     {XX}   Page Or Account {RW}")
    print(f"       {BG}{B3}  Reaction For Comments     {F}   [ {Z1}11{F} ]     {XX}   Page Or Account {RW}")
    print(f"       {BG}{B3}  Auto Reply Comments        {F}  [ {Z1}12 {F}]      {XX}  Page Or Account {RW}")
    print(f"       {BG}{B3}  Reaction Post Love/Wow     {F}  [{Z1} 13{F} ]     {XX}   Page Or Account {RW}")
    print(f"       {BG}{B3}  Group Join                {F}   [ {Z1}14 {F}]   {XX}Unstable but working { RW}")
    print(f"       {BG}{B3}  Reaction For Live           {F} [ {Z1}15 {F}]        {XX}Page Or Account {RW}") 
    print(f"       {BG}{B4}  Custom Comment for Live      {F}[ {Z1}16{F} ]     {XX}   Page Or Account {RW}") 
    print(f"       {BG}{B4}  Reaction Post W/0 Care     {F}  [ {Z1}17{F} ]       {XX} Page Or Account {RW}") 
    print(f"       {BG}{B4}  Auto Create Page           {F}  [ {Z1}18 {F}]       {XX} Random Set Up   {RW}") 
    print(f"       {BG}{B4}  Auto Set PFP                 {F}[ {Z1}19 {F}]     {XX}   Random Set Up   {RW} ")
    print(f"       {BG}{B4}  Create File                {F}  [ {Z1}20 {F}]      {XX}  Random Set Up   {RW} ")
    print(f"       {BG}{B4}  Delete Duplicate Account     {F}[{Z1} 21 {F}]     {XX}   Remover         {RW} ")
    print(f"       {BG} {B5} Live Account Checking       {F} [ {Z1}22 {F}]        {XX}Checker         {RW} ")
    print(f"       {BG} {B5} Account Token Checker       {F} [ {Z1}23 {F}]    {XX}  Validator/Checker {RW}")
    print(f"       {BG}{B5}  Account Redeem            {F}   [{Z1} 24 {F}]    {XX} Account Redemption {RW}")
    print(f"       {BG} {B5} Auto Share Method 1         {F} [ {Z1}25 {F}]        {XX}Bot Share       {RW} ")
    print(f"       {BG} {B6} Auto Share Method 2         {F} [ {Z1}26 {F}]     {XX}   Bot Share       {RW} ")
    print(f"       {BG} {B6} Auto Facebook Views         {F} [ {Z1}27 {F}]    {XX}    Cookies         {RW} ")
    print(f"       {BG} {B6} Get Account Token            {F}[ {Z1}28 {F}]     {XX}   Normal Account  {RW} ")
    print(f"       {BG}  {B6}Facebook Auto Report      {F}   [ {Z1}29{F} ]  {XX}      Mass Report     {RW} ")
    print(f"       {BG}  {B6}Auto Tiktok Views           {F} [ {Z1}30 {F}]     {XX}   Other Service   {RW} ")
