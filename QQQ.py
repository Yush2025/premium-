...
def jovan():
    ...
    print(f"""
{BB} ╭───────────────────────────────────────────────────────────────────────╮ 
 {BB}│              {Q}▄▄▄▄   {AA} ▒{Q}█████ {Q}  ▒{Q}█████    {Q}██████{Q} ▄▄▄█████{AA}▓              {BB}│
{BB} │            {AA} ▓{Q2}█████▄{AA} ▒{Q2}██{AA}▒  {Q2}██{AA}▒▒{Q2}██{AA}▒ {Q2} ██{AA}▒▒{Q2}██   {AA} ▒ ▓  {Q2}██{AA}▒ ▓▒           {BB}   │
{BB} │             {AA}▒{Q3}██{AA}▒ {Q3}▄██{AA}▒{Q3}██{AA}░  {Q3}██{AA}▒▒{Q3}██{AA}░  {Q3}██{AA}▒░ ▓{Q3}██▄   {AA}▒ ▓{Q3}██{AA}░ ▒░            {BB}  │
{BB} │             {AA}▒{Q4}██{AA}░{Q4}█▀ {AA} ▒{Q4}██   ██{AA}░▒{Q4}██  {Q4} ██{AA}░  ▒   {Q4}██{AA}▒░ ▓{Q4}██{AA}▓ ░              {BB} │
{BB} │            {AA} ░▓{Q5}█  ▀█{AA}▓░ {Q5}████{AA}▓▒░░ {Q5}████{AA}▓▒░▒{Q5}██████{AA}▒▒  ▒{Q5}██{AA}▒ ░           {BB}    │
{BB} │          {AA}   ░▒▓{Q6}███▀{AA}{rb}{AA}▒░ ▒░▒░▒░ ░ ▒░▒░▒░ ▒ ▒▓▒ ▒ ░  ▒ ░░                 {BB}│
{BB} │            {AA} ▒░▒   ░   ░ ▒ ▒░   ░ ▒ ▒░ ░ ░▒  ░ ░    ░ {Z1} Version 5.1.1 {BB}  │
{BB} │            {AA}  ░    ░ ░ ░ ░ ▒  ░ ░ ░ ▒  ░  ░  ░    ░                   {BB} │
{BB} │            {AA}  ░          ░ ░      ░ ░        ░                        {BB} │
{BB} │                 {AA}  ░                                                  {BB} │
...
def detact_method():
	...
	print(f"[2] METHOD API (SLOW)")
	
	choice = input('\x1b[1;32mCHOOSE : ')
	if choice == '1':
		ApiSystem().main(method=1)
	if choice == "2":
		ApiSystem().main(method=2)
	else:
		print(f"{red}INVALID OPTION.")
		detact_method()

class ApiSystem:
    def __init__(self, orgin=None):
        self.X = orgin
        self.save = []
        self.loop = 0
        self.oks = []
        self.cps = []
        self.ers = []
        self.path = "/sdcard/ERROR-BOOSTING"
    ...
    def main(self,method):
        ...
        print(f"YOUR FILE FORMAT?")
        

        print(f"[1] UID|PASS")
        print(f"[2] UID|PASS|COOKIE")
        print(f"[3] EMAIL|PASS")
        print(f"[4] EMAIL|PASS|COOKIE")
        
        choice = input('\x1b[1;32mCHOOSE : ')
        if choice == "1":self.mailpass(method)
        elif choice == "2":self.mailpasscokie(method)
        elif choice == "3":self.mailpass(method)
        elif choice == "4":self.mailpasscokie(method)
        else:self.main(method)
    
    def mailpass(self,method):
        clear()
        
        self.file = input(f"PUT FILE PATH : ")
        try:self.files = open(self.file,"r").read().splitlines()
        except FileNotFoundError:self.mailpass(method)
        
        print(f"SAVE FILE FORMAT?")
        
        print(f"[1] UID|PASS")
        print(f"[2] UID|PASS|COOKIE")
        print(f"[3] UID|EMAIL|PASS|COOKIE")
        
        choice = input('\x1b[1;32mCHOOSE : ')
        if choice == "1":self.save.append("A")
        elif choice == "2":self.save.append("B")
        elif choice == "3":self.save.append("C")
        clear()
        
        print(f"TOTAL IDS : {green}{str(len(self.files))}")
        print(f"PROCCESS HAS BEEN STARTED....")
        ...
        print(f"TOTAL OKS :- {green}{str(len(self.oks))}")
        print(f"TOTAL CPS :- {red}{str(len(self.cps))}")
        print(f"TOTAL ERS :- {yellow}{str(len(self.ers))}")
    ...
    def apimethodA(self,email,password):
        global loop,oks,cps
        sys.stdout.write(f"\r {white}[ERROR-M1]-[{self.loop}]-[OKS:-{green}{len(self.oks)}{white}]\r")
        sys.stdout.flush()
        try:
            with requests.Session() as session:
                ...
                if "access_token" in response:
                    ...
                    if "Photoshop" in lock_rm:
                        ...
                        if "A" in self.save:
                            print(f"\r\r {green}[SUCCESSFULLY] {uid}{white}|{green}{password}")
                            paths = "/sdcard/EXTRACT-M1-OK.txt"
                            with open(paths,"a") as saves:
                                saves.write(uid+"|"+password+"\n")
                            self.oks.append(uid)
                        elif "B" in self.save:
                            print(f"\r\r {green}[SUCCESSFULLY] {uid}{white}|{green}{password}{white}|{green}{cookie}");linex()
                            paths = "/sdcard/EXTRACT-M1-OK.txt"
                            with open(paths,"a") as saves:
    ...
    def apimethodB(self,email,password):
        global loop,oks,cps
        sys.stdout.write(f"\r {white}[ERROR-M2]-[{self.loop}]-[OKS:-{green}{len(self.oks)}{white}]\r")
        sys.stdout.flush()
        try:
            with requests.Session() as session:
...
def main():
    ...
    jovan()
    print(f"""{BB} ┌──────────────────────────────────────────────────────────────────────────┐
{BB} │   🗂️ Total of {green}FRA{white} ACCOUNT Stored  : {total_accounts}                                  {BB}  │
{BB} │   🗂️ Total of {green}FRA {white}PAGE  Stored    : {total_pages}                               {BB}      │
{BB} │   🗂️ Total of {red}RPW {white}ACCOUNT Stored  : {total_account_rpw}                                  {BB}   │
{BB} │   🗂️ Total of {red}RPW {white}PAGE Stored     : {total_pages_rpw}                                {BB}     │
{BB} │                                                                        {BB}  │
{BB} │              {green}A C C O U N T  O V E R V I E W  S T A T U S               {white} {BB} │
{BB} └──────────────────────────────────────────────────────────────────────────┘""")

    print(f" {white}TOOLS {red}ONHOLD ")
    print(f" {white}Time set : {green}9am ")
    print(f" {white}Date : Sept {red}10 {white}wed. ")
    print(f" {white}Reason : {red}No payment Recieved During Transaction Time ")
    print(f" {green}Tools well Activated once already Paid ")
    print(f" {white}Contact {green}@Xiannn13{white} in telegram ") 
